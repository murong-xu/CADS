...
def generate_latex_table(metric_data_1_dict, metric_data_2_dict, metric_type, output_path):
    ...
    # row OMASeg
    omaseg_values = [
        metric_1_data[('in', 'OMASeg', 'overlapping')],
        metric_2_data[('in', 'OMASeg', 'overlapping')],
        metric_1_data[('out', 'OMASeg', 'overlapping')],
        metric_2_data[('out', 'OMASeg', 'overlapping')],
        metric_1_data[('all', 'OMASeg', 'overlapping')],
        metric_2_data[('all', 'OMASeg', 'overlapping')]
    ]
    mutual_rows.append("& OMASeg & " + " & ".join(format_value(v, show_percentage=p) for v, p in zip(omaseg_values, show_percentages)) + " \\\\")
    ...
    all_values = [
        ...
        metric_2_data[('in', 'OMASeg', 'all')],
        metric_1_data[('out', 'OMASeg', 'all')],
        metric_2_data[('out', 'OMASeg', 'all')],
        metric_1_data[('all', 'OMASeg', 'all')],
        metric_2_data[('all', 'OMASeg', 'all')]
    ]

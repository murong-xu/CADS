...
def main():
    ...
    ctready_folder = '/net/cephfs/shares/menze.dqbm.uzh/murong/20k/shape_mismatch_correction/Verse/verse_4xx_cases_original'
    ...
    original_img_folder = os.path.join(ctready_folder)
    ...
    for i, img_id in enumerate(img_ids):
        ...
        original_img_file = os.path.join(original_img_folder, 'images', img_id+'.nii.gz')

        if not os.path.exists(original_img_file):
            print(f"Original image file {original_img_file} does not exist. Skipping...")
            continue
